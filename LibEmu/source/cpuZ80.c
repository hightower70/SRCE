...
/** forget to provide cpuMemRead(), cpuMemWrite(), InZ80(), OutZ80(),  **/
...
/*************************************************************/
#ifdef COLEM
#define cpuMemRead cpuMemRead
extern byte *ROMPage[];
INLINE byte cpuMemRead(word A) { return(ROMPage[A>>13][A&0x1FFF]); }
#endif

#ifdef SPECCY
#define cpuMemRead cpuMemRead
#define cpuMemWrite cpuMemWrite
extern byte *Page[],*ROM;
INLINE byte cpuMemRead(word A)        { return(Page[A>>13][A&0x1FFF]); }
INLINE void cpuMemWrite(word A,byte V) { if(Page[A>>13]<ROM) Page[A>>13][A&0x1FFF]=V; }
#endif
...
/** FAST_RDOP ************************************************/
/** With this #define not present, cpuMemRead() should perform   **/
/** the functions of OpZ80().                               **/
/*************************************************************/
#ifndef FAST_RDOP
#define OpZ80(A) cpuMemRead(A)
#endif
...
#define M_PUSH(Rg)     \
  cpuMemWrite(--R->SP.W,R->Rg.B.h);cpuMemWrite(--R->SP.W,R->Rg.B.l)

#define M_CALL         \
  J.B.l=OpZ80(R->PC.W++);J.B.h=OpZ80(R->PC.W++);         \
  cpuMemWrite(--R->SP.W,R->PC.B.h);cpuMemWrite(--R->SP.W,R->PC.B.l); \
  R->PC.W=J.W; \
  JumpZ80(J.W)
...
#define M_RST(Ad)      \
  cpuMemWrite(--R->SP.W,R->PC.B.h);cpuMemWrite(--R->SP.W,R->PC.B.l);R->PC.W=Ad;JumpZ80(Ad)
...
static void CodesCB(register cpuZ80State *R)
{
  register uint8_t I;
...
static void CodesDDCB(register cpuZ80State *R)
{
  register pair J;
  register uint8_t I;
...
static void CodesFDCB(register cpuZ80State *R)
{
  register pair J;
  register uint8_t I;
...
static void CodesED(register cpuZ80State *R)
{
  register uint8_t I;
  register pair J;
...
static void CodesDD(register cpuZ80State *R)
{
  register uint8_t I;
  register pair J;
...
static void CodesFD(register cpuZ80State *R)
{
  register uint8_t I;
  register pair J;
...
/*************************************************************/
int cpuExecute(register cpuZ80State *R,register int in_cycles_requested)
{
  register uint8_t I;
  register pair J;
	int cycles_executed;
  ...
  while(R->ICount < in_cycles_requested)
  {
#ifdef cpuZ80_DEBUG_ENABLED
    /* Turn tracing on when reached trap address */
    //if(R->PC.W==R->Trap) R->Trace=1;
    /* Call single-step debugger, exit if requested */
    if(R->Trace)
      if(!cpuZ80Debug(R)) return(R->ICount);
#endif
...
#include "cpuZ80Codes.h"
  ...
  else
  {
    ...
    R->IFF=(R->IFF&~IFF_EI)|IFF_1;
		...
		if ((R->IRequest != INT_NONE) && (R->IRequest != INT_QUIT))
		{
			cpuInt(R, R->IRequest);
		}
  }
...
/*************************************************************/
void cpuInt(cpuZ80State *R,uint16_t in_vector)
{
  ...
  if((R->IFF&IFF_1)||(in_vector==INT_NMI))
  {
    /* Save PC on stack */
    M_PUSH(PC);

    /* Automatically reset IRequest if needed */
    if(R->IAutoReset&&(in_vector==R->IRequest)) 
			R->IRequest=INT_NONE;

    /* If it is NMI... */
    if(in_vector==INT_NMI)
    {
      /* Clear IFF1 */
      R->IFF&=~(IFF_1|IFF_EI);
    ...
    if(R->IFF&IFF_IM2)
    {
      ...
      in_vector=(in_vector&0xFF)|((uint16_t)(R->I)<<8);
      /* Read the vector */
      R->PC.B.l=cpuMemRead(in_vector++);
      R->PC.B.h=cpuMemRead(in_vector);
      JumpZ80(R->PC.W);
      /* Done */
      return;
    }
