...
case DB_FE:     cpuZ80Patch(R);break;
/*************************************************************/
...
case LD_xWORDe_DE:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  cpuMemWrite(J.W++,R->DE.B.l);
  cpuMemWrite(J.W,R->DE.B.h);
  break;
case LD_xWORDe_BC:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  cpuMemWrite(J.W++,R->BC.B.l);
  cpuMemWrite(J.W,R->BC.B.h);
  break;
case LD_xWORDe_SP:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  cpuMemWrite(J.W++,R->SP.B.l);
  cpuMemWrite(J.W,R->SP.B.h);
  break;
...
case LD_DE_xWORDe:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  R->DE.B.l=cpuMemRead(J.W++);
  R->DE.B.h=cpuMemRead(J.W);
  break;
case LD_BC_xWORDe:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  R->BC.B.l=cpuMemRead(J.W++);
  R->BC.B.h=cpuMemRead(J.W);
  break;
case LD_SP_xWORDe:
  J.B.l=OpZ80(R->PC.W++);
  J.B.h=OpZ80(R->PC.W++);
  R->SP.B.l=cpuMemRead(J.W++);
  R->SP.B.h=cpuMemRead(J.W);
  break;

case RRD:
  I=cpuMemRead(R->HL.W);
  J.B.l=(I>>4)|(R->AF.B.h<<4);
  cpuMemWrite(R->HL.W,J.B.l);
...
case OUT_xC_B: cpuOut(R->BC.W,R->BC.B.h);break;
case OUT_xC_C: cpuOut(R->BC.W,R->BC.B.l);break;
case OUT_xC_D: cpuOut(R->BC.W,R->DE.B.h);break;
case OUT_xC_E: cpuOut(R->BC.W,R->DE.B.l);break;
case OUT_xC_H: cpuOut(R->BC.W,R->HL.B.h);break;
case OUT_xC_L: cpuOut(R->BC.W,R->HL.B.l);break;
case OUT_xC_A: cpuOut(R->BC.W,R->AF.B.h);break;

case INI:
	cpuMemWrite(R->HL.W++,cpuIn(R->BC.W));
  --R->BC.B.h;
...
case INIR:
  ...
  while(R->BC.B.h&&(R->ICount<R->ICyclesRequested));
...
case LDI:
  cpuMemWrite(R->DE.W++,cpuMemRead(R->HL.W++));
  --R->BC.W;
...
case LDIR:
  ...
  while(R->BC.W&&(R->ICount<R->ICyclesRequested));
...
case CPIR:
  ...
  while(R->BC.W&&J.B.l&&(R->ICount<R->ICyclesRequested));
  R->AF.B.l =
    N_FLAG|(R->AF.B.l&C_FLAG)|ZSTable[J.B.l]|
    ((R->AF.B.h^I^J.B.l)&H_FLAG)|(R->BC.W? P_FLAG:0);
