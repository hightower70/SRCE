...
static volatile USB_ENDPOINT_INFO   *pCurrentEndpoint[USB_MAX_ADDRESS+1] = {0};           // Pointer to the endpoint currently performing a transfer.
...
static BOOL USBHostInit(  BYTE deviceAddress  );
...
void USBHostInitialize(void)
{
	BYTE deviceAddress;

	for(deviceAddress = 0; deviceAddress <= USB_MAX_ADDRESS; deviceAddress++)
	{
		usbDeviceInfo[deviceAddress].pEndpoint0 = NULL;
		pCurrentEndpoint[deviceAddress] = NULL;
	}
...
void _USB_CheckCommandAndEnumerationAttempts( BYTE deviceAddress )
{
	...
	pEndpoint = (USB_ENDPOINT_INFO*) pCurrentEndpoint[deviceAddress];
	//  DEBUG_PutChar( '=' );
...
/*	DEBUG_PutString("HOST: _USB_FindClassDriver(bClass=0x"); DEBUG_PutHexUINT8(bClass);
...
void _USB_SendToken( BYTE deviceAddress, BYTE endpoint, BYTE tokenType )
{
	...
	if (pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_CONTROL)
	{
		temp &= 0xEF;   // Clear EPCONDIS
	}

	// Disable handshaking for isochronous endpoints.
	if (pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
	{
...
void _USB_SetBDT( BYTE deviceAddress, BYTE token )
{
	...
	{
		if ((pEndpoint->dataCountMaxSETUP - pEndpoint->dataCount) > pEndpoint->wMaxPacketSize)
		{
			currentPacketSize = pEndpoint->wMaxPacketSize;
		}
		else
		{
			currentPacketSize = pEndpoint->dataCountMaxSETUP - pEndpoint->dataCount;
		}
	}
	else
	{
		...
		{
			if ((pEndpoint->dataCountMax - pEndpoint->dataCount) > pEndpoint->wMaxPacketSize)
			{
				currentPacketSize = pEndpoint->wMaxPacketSize;
			}
			else
			{
				currentPacketSize = pEndpoint->dataCountMax - pEndpoint->dataCount;
			}
		}
	}
	...
	if (token == USB_TOKEN_SETUP)
	{
		...
		pBDT->ADR  = ConvertToPhysicalAddress(pEndpoint->pUserDataSETUP);
#else
#error Cannot set BDT address.
	...
	else
	{
		...
		if (pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
		{
			pBDT->ADR  = ConvertToPhysicalAddress(((ISOCHRONOUS_DATA *) (pEndpoint->pUserData))->buffers[((ISOCHRONOUS_DATA *) (pEndpoint->pUserData))->currentBufferUSB].pBuffer);
		}
		else
		{
			pBDT->ADR  = ConvertToPhysicalAddress((WORD) pEndpoint->pUserData + (WORD) pEndpoint->dataCount);
		}
#elif defined(__PIC32MX__)
		if (pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS)
		{
		...
		{
			pBDT->ADR  = ConvertToPhysicalAddress((DWORD) pEndpoint->pUserData + (DWORD) pEndpoint->dataCount);
		}
#else
#error Cannot set BDT address.
#endif
	...
	pBDT->STAT.DTS      = pEndpoint->status.bfNextDATA01;
	pBDT->STAT.DTSEN    = pEndpoint->status.bfUseDTS;
...
#error Need structure name for copyU1STATbits.
		...
		if (!(U1IEbits.UERRIE && U1IRbits.UERRIF) || (pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS))
		{
			...
			// at U1TOK, or trust that pEndpoint is still accurate.
			if ((pEndpoint->bEndpointAddress & 0x0F) == (U1TOK & 0x0F))
			{
				...
				if (pBDT->STAT.PID == PID_ACK)
				{
					...
					pEndpoint->dataCount += packetSize;

					// Set the NAK retries for the next transaction;
					pEndpoint->countNAKs = 0;

					// Toggle DTS for the next transfer.
					pEndpoint->status.bfNextDATA01 ^= 0x01;

					if ((pEndpoint->transferState == (TSTATE_CONTROL_NO_DATA | TSUBSTATE_CONTROL_NO_DATA_SETUP)) ||
						(pEndpoint->transferState == (TSTATE_CONTROL_READ    | TSUBSTATE_CONTROL_READ_SETUP)) ||
						(pEndpoint->transferState == (TSTATE_CONTROL_WRITE   | TSUBSTATE_CONTROL_WRITE_SETUP)))
					{
						// We are doing SETUP transfers. See if we are done with the SETUP portion.
						if (pEndpoint->dataCount >= pEndpoint->dataCountMaxSETUP)
						{
					...
					else
					{
						...
						if ((pEndpoint->bmAttributes.bfTransferType == USB_TRANSFER_TYPE_ISOCHRONOUS) ||
							(packetSize < pEndpoint->wMaxPacketSize) ||
							(pEndpoint->dataCount >= pEndpoint->dataCountMax))
						{
							// We've written all the data. Proceed to the next step.
							pEndpoint->status.bfTransferSuccessful = 1;
							_USB_SetNextTransferState(deviceAddress);
						}
						else
						{
				...
				else if (pBDT->STAT.PID == PID_NAK)
				{
					...
					pEndpoint->status.bfLastTransferNAKd = 1;
#endif

					pEndpoint->countNAKs ++;

					switch ( pEndpoint->bmAttributes.bfTransferType )
					{
						case USB_TRANSFER_TYPE_BULK:
							...
							if (pEndpoint->status.bfNAKTimeoutEnabled &&
								(pEndpoint->countNAKs > pEndpoint->timeoutNAKs))
							{
								pEndpoint->status.bfError    = 1;
								pEndpoint->bErrorCode        = USB_ENDPOINT_NAK_TIMEOUT;
								_USB_SetTransferErrorState( pEndpoint );
							}
							break;
						...
						case USB_TRANSFER_TYPE_INTERRUPT:
							if ((pEndpoint->bEndpointAddress & 0x80) == 0x00)
							{
				...
				else if (pBDT->STAT.PID == PID_STALL)
				{
					...
					pEndpoint->status.bfStalled = 1;
					pEndpoint->bErrorCode       = USB_ENDPOINT_STALLED;
					_USB_SetTransferErrorState( pEndpoint );
				}
				else
				{
					...
					pEndpoint->status.bfErrorCount++;

					if (pEndpoint->status.bfErrorCount >= USB_TRANSACTION_RETRY_ATTEMPTS)
					{
						...
						pEndpoint->status.bfError    = 1;
						pEndpoint->bErrorCode        = USB_ENDPOINT_ERROR_ILLEGAL_PID;
						_USB_SetTransferErrorState( pEndpoint );
	...
	if (U1IEbits.UERRIE && U1IRbits.UERRIF)
	{
		...
		else
		{
			...
			if (pEndpoint->status.bfErrorCount >= USB_TRANSACTION_RETRY_ATTEMPTS)
			{
				...
				if (U1EIRbits.BTSEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_BIT_STUFF;
				if (U1EIRbits.DMAEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_DMA;
				if (U1EIRbits.BTOEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_TIMEOUT;
				if (U1EIRbits.DFN8EF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_DATA_FIELD;
				if (U1EIRbits.CRC16EF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_CRC16;
				if (U1EIRbits.EOFEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_END_OF_FRAME;
				if (U1EIRbits.PIDEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_PID_CHECK;
#if defined(__PIC32MX__)
				if (U1EIRbits.BMXEF)
					pEndpoint->bErrorCode = USB_ENDPOINT_ERROR_BMX;
#endif
